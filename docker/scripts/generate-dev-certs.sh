...
echo "Generating self-signed TLS cert for local production preview..."
openssl req -x509 -nodes -days 365 -newkey rsa:4096 \
  -keyout "$SSL_DIR/privkey.pem" \
  -out "$SSL_DIR/fullchain.pem" \
  -subj "/CN=localhost/O=FSTEC Dev" \
  -addext "subjectAltName=DNS:localhost,DNS:fstec.local,IP:127.0.0.1"
