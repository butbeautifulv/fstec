#!/usr/bin/env node
/**
 * Extract xlsx history (measure, subdivision, result) from corpus reports.
 * Usage: npx tsx scripts/extract-corpus-history.mjs [--out corpus-history.jsonl]
 */
...
async function main() {
  ...
  const outPath =
    outArg >= 0 ? process.argv[outArg + 1] : join(ROOT, "corpus-history.jsonl")
  ...
  console.log(JSON.stringify({ historyRows: lines.length, reportFiles: fileCount, outPath }))
}
