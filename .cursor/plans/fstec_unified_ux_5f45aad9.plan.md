...
| ДЗО — разорванный flow | Подразделения на [`/panel/organizations`](app/(admin)/admin/(panel)/organizations/page.tsx), ссылки на [`/panel/organizations/[id]`](app/(admin)/admin/(panel)/organizations/[id]/page.tsx) — два экрана без связи |
...
2. **Measure edit refresh** — в PUT/POST measures API: `revalidatePath('/panel/measures')`; в [`measure-form.tsx`](components/admin/measure-form.tsx): `toast.success('Мера сохранена')` перед redirect
...
List page `/panel/organizations` — только таблица ДЗО, клик по строке → `/panel/organizations/[id]`.
...
- Sidebar [`admin-sidebar.tsx`](components/admin/admin-sidebar.tsx): active state для nested routes (`/panel/organizations/123` → ДЗО active)
