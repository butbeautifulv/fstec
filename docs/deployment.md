...
Production: задайте в [`.env.production.example`](../.env.production.example) и `.env.production`:

| Переменная | Назначение |
|------------|------------|
| `APP_URL` | Публичный URL для ссылок в письмах |
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM` | Исходящая почта |
| `OPERATOR_NOTIFY_EMAIL` | Email оператора для уведомлений |
| `CRON_SECRET` | Bearer-токен для `/api/cron/*` |
| `INBOX_IMAP_*` | Автоимпорт DOCX из входящей почты (опционально) |
